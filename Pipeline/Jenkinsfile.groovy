  ...
  stages {
    stage('PowerShell via Ansible') {
      steps {
        ...
        If [[ $params.System -eq "Windows" ]]
        Then sh "ansible-playbook Pipeline/Get-Service.yml -e 'ServiceName=$params.ServiceName State=$params.State StartType=$params.StartType'"
        Else echo "Linux"
        Fi
      }
    }
